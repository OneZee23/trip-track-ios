...
### v0.6.2 — своё лицо (в работе)

- [x] Пробег машины выводится из поездок, а не накапливается: смена авто у поездки, удаление и восстановление с сервера пересчитывают оба одометра
- [x] Аватар машины перестал быть единственным размытым спрайтом — `.interpolation(.none)` в `Vehicle.avatarView`
- [x] Имя аватара свёрнуто в `TripTrackShared/VehicleAvatar.swift`; пять копий `hasPrefix("pixel_car_")` в двух таргетах печатали бы сырую строку при первом же не-легковом имени
- [x] Тесты соответствия каталогов ассетов двух таргетов — спрайт, забытый в Live Activity, раньше находился только на экране блокировки
- [x] Иллюстрации пустых состояний, значки категорий достижений, метки старта и финиша
- [ ] Набор транспорта: кузова и два колеса отдельными силуэтами, цвет отдельной осью
- [ ] Пикер на две оси в форме машины
- [ ] Новые скриншоты и промо-текст в App Store
