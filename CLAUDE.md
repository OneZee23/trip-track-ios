...
- **Геометрия таб-бара — в `CustomTabBar` и только там.** `bottomLift(bottomInset:)`:
  ...
  → 24 pt, без индикатора — канонные 14; `clearance` = пилюля + подъём + 8;
  внутри `NavigationStack` (профиль, гараж, паспорт, лента) —
  `clearanceAboveSafeArea`: скролл там кончается на границе безопасной зоны,
  а пилюля стоит от физического низа — им уводят последнюю строку экраны,
  над которыми бар ВИДЕН (лента, карта (карточка `MyMapSheet`), «Места»,
  гараж, паспорт машины, экраны клубов).
