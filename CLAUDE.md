...
- **Снятая галочка хранится, а не подразумевается.** Окно берёт ВСЕ свои
  поездки внутри границ, поэтому `JourneyManager.create(from:)` пишет соседей,
  которых человек снял в листе, в `excludedTripIds`. Иначе они возвращались бы
  плечами сразу после сохранения — список приходил отфильтрованным, а в базу
  уезжали одни даты. «Убрать из путешествия» (долгое нажатие на плече) — тот же
  список, а не сдвиг границ: убранная поездка бывает в середине окна.
