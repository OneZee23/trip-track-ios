...
- **Слот «Finnish» — это филиппинский.** Финского языка в приложении нет; при
  заведении локалей в App Store Connect `fil` спутали с `fi`. Слот НЕ
  переименовываем, кладём в него ФИЛИППИНСКИЙ текст — так делали в 0.6.3 и
  0.6.4. Финский пользователь получит финскую карточку и англоязычное
  приложение; это известно и принято.
...
Впервые записано в `docs/releases/0.6.2/app-store-locales.md`, повторено в
`docs/releases/0.6.4/app-store.md`.
