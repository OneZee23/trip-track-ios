...
живёт в `trip-track-backend`. **Бэкенд деплоится ПЕРВЫМ, до сабмита**: push
master → GitLab CI задеплоит сам, колонку `last_used_at` прод создаст при
старте контейнера (`DB_SYNC=true`), миграция в репо — guarded, на всякий.
Порядок безопасен в обе стороны: старый клиент со свежим бэкендом просто
перестаёт разлогиниваться, новый клиент со старым бэкендом чинит себя
повторами в пределах прежних 60 секунд. Сайт трогать не нужно. Колонка
`avatar_style` на сервере полезна, но не обязательна — без неё чужие машины
приезжают легковыми, ровно как сегодня.

**В карточке ДВЕНАДЦАТЬ локализаций, и «What's New» обязателен в каждой.**
Сабмит 0.6.2 был отклонён именно из-за этого (01.09): App Store Connect не
...
и, по желанию, промо-текст.

⚠️ Finnish в карточке есть, а финского языка в приложении НЕТ — тринадцатый
язык у нас Filipino (`fil`). Похоже на путаницу `fil`/`fi` при заведении
локализаций. На сабмит не влияет, но разобрать стоит; казахского (`kk`) в
карточке нет вовсе.
...
A dropped connection at the start of a drive could sign you out overnight and
leave the finished trip stranded on the phone. The app now repairs its session
on its own — and if it truly can't, it keeps your trips, your name and your
settings, and simply asks you to sign in again. Nothing is wiped.
...
Обрыв связи в начале поездки мог за ночь разлогинить аккаунт, а записанную
поездку оставить на телефоне. Теперь приложение само восстанавливает сессию, а
если это правда невозможно — сохраняет поездки, имя и настройки и просто
просит войти ещё раз. Ничего не стирается.
...
0. **Запушить бэкенд** (`trip-track-backend`, master → origin): CI задеплоит,
   `DB_SYNC=true` создаст `last_used_at` при старте. Проверить после деплоя:
   `/auth/refresh` живой (любой юзер в логах), и в логах появилась строка
   «honored as rotated replay» ИЛИ хотя бы нет всплеска INVALID_REFRESH_TOKEN.
...
9. **Смоук мягкого истечения сессии** (5 минут): на залогиненном устройстве
   стереть refresh-токен из связки (или дождаться `INVALID_REFRESH_TOKEN` на
   дев-стенде) → на «Я»-табе карточка «Нужно войти снова», имя в шапке НЕ
   «Гость», поездки на месте; вход через Apple — синк доливает очередь.
