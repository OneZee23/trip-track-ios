...
- [x] **Photos or Videos**
  ...
  - Linked to user? **Yes**
  - Used for tracking? **No**
  - 0.6.4: к фотографиям ПОЕЗДКИ добавились фотографии МАШИНЫ. Класс данных тот
    же, поэтому новую строку заводить не нужно; изменилось другое — снимки
    машины уезжают на сервер при включённой синхронизации, как и снимки
    поездок, и так же остаются на устройстве, пока она выключена.
- [x] **Other User Content**
  - Used for: App Functionality
  - Linked to user? **Yes**
  - Used for tracking? **No**
  - Описание: названия поездок, заметки, профили машин, отображаемое имя в социальных функциях
  - 0.6.4: сюда же попадают марка, модель, год, строка «про машину» и
    РЕГИСТРАЦИОННЫЙ НОМЕР. Номер вводится по желанию, по умолчанию скрыт от
    других и никогда не покидает сервер для типов транспорта, у которых номера
    не бывает. Отдельного класса «Sensitive Info» он не требует: это не
    документ, удостоверяющий личность, и Apple относит его к пользовательскому
    контенту — но упомянуть его в заметках ревьюеру обязательно (сделано).
