...
## Обновление 7 сентября 2026 (по итогам аудита, не юрконсультация)

- **Рейтинг: 13+**, не 4+. Публичная лента с реакциями и комментариями — «Social Media» по
  таблице Apple; с сентября 2026 анкета обязательна при любом сабмите.
- **Сделать сейчас, полдня:** уведомление РКН как оператор ПДн (Госуслуги; штраф физлицу за
  отсутствие 5–10 тыс. ₽ и «индикатор риска» для проверки) + уведомление о трансграничной
  передаче в Германию (страна «адекватная», можно сразу после подачи; нужны сведения о мерах
  защиты от DigitalOcean/Cloudflare — есть в DPA).
- **Локализация с 01.07.2025:** хранение ПДн россиян только во Франкфурте формально нарушает
  ч.5 ст.18 152-ФЗ; физлицу 30–50 тыс. ₽; дел против инди-разработчиков в открытых источниках
  нет. Решение — при росте аудитории: первичная БД в РФ + реплика в ЕС.
- **GDPR:** политика по Art.13 на EN/DE, реестр Art.30; представитель по Art.27 — отложить
  (risk-based). **DSA** (ст.13 представитель) и **UK OSA** размерных исключений не имеют —
  проще не таргетить UK.
- «Приземление» 236-ФЗ — только >500 тыс. посетителей в сутки, физлиц не касается.
