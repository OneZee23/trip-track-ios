...
/// Tiny floating-icon effect that fires when a reaction goes from
/// off → on. Animates a copy of the drawn reaction icon (fill variant,
/// accent) upward + outward, scaling up while fading out, over ~0.7s.
/// Pure presentational; the parent owns the lifetime via `Set<String>`
/// membership and removes the view when the animation finishes.
///
...
struct ReactionBurstSprite: View {
    ...
    var body: some View {
        ReactionIconView(emoji: emoji, size: 30, filled: true)
