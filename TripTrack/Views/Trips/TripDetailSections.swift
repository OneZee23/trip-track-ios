...
enum TripDetailFormat {
    ...
    /// «1 310» — группирование для любого крупного целого ЧИСЛА, у которого
    /// нет единицы расстояния.
    ///
    /// Километры путешествия ходили здесь же — и это была ошибка, которую
    /// компилятор не мог поймать: как только расстояние стало переводиться,
    /// «сгруппируй число» и «покажи расстояние» разошлись, а сигнатура у них
    /// одна. Расстояние теперь показывает `Measure`, здесь остались деньги.
