    ...
    private func progressBlock(
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(alignment: .top, spacing: 8) {
                Text(isMaxLevel
                     ? AppStrings.levelsXpTotal(l, xp: AppStrings.groupedNumber(xp, l))
                     : AppStrings.levelsXpProgress(
                        l,
                        current: AppStrings.groupedNumber(xp, l),
                        target: AppStrings.groupedNumber(LevelSystem.xpForNextLevel(level), l)
                     ))
