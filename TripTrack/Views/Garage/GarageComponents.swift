...
enum GarageFormat {
    // `odometer(_:lng:)` здесь больше нет, и это не уборка, а ловушка,
    // обезвреженная 0.6.7: последними её звали ТРИ места, и все три печатали
    // ею ВЫСОТУ, а не пробег. Встроенная в функцию с таким именем конверсия
    // увезла бы метры в мили коэффициентом 0.621 без единой ошибки
    // компиляции. Пробег печатает `Measure.odometer(km:)`, высоту —
