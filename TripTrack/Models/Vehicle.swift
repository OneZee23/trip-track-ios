...
struct Vehicle: Identifiable, Codable {
    ...
    /// В каком диалекте показывать и разбирать расход ЭТОЙ машины.
    ///
    /// Выводится из приборки, а не настраивается отдельной галочкой:
    /// метрическая панель — литры на сотню километров, мильная — мили на
    /// галлон. Отдельная галочка давала бы «литры на сотню МИЛЬ» — единицу, в
    /// которой не ездит никто, зато число в ней молча другое. См.
    /// `ConsumptionUnit`.
    ///
    /// Единица человека приходит параметром по той же причине, что и у
    /// `dashboardUnit(app:)`: сумме по нескольким машинам эту функцию нечем
    /// позвать.
