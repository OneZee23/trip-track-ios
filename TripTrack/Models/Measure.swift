...
///
/// **Разделитель и разряды — отсюда, а не с места вызова.** Само число печатает
/// `UnitNumber` (папка `TripTrackShared`), туда же за ним ходят `AppStrings` и
/// виджет на локскрине — одна таблица на оба процесса. Вызывающему поэтому
/// нечего писать через `String(format:)`, а вместе с этим уходят живые баги
/// локали: разряды по языку и ОДИН на всё приложение десятичный разделитель —
/// точка (решение владельца, см. `UnitNumber.decimalSeparator`).
enum Measure {
    ...
    /// вёрстку.
    enum Style {
        /// Всегда одна десятая, даже если она ноль: карточки ленты, итог
        /// поездки, плитка «Дистанция», плашка REC, восстановление, постер.
        ///
        /// «Даже если ноль» — это не небрежность, а работа стиля. Половина его
        /// мест — ЖИВОЙ счётчик (плашка REC, плитка записи, реплей), и ширина
        /// числа там не должна прыгать на символ каждый километр.
        case tenths
        /// Целое с разрядами по языку: одометр, статистика, регионы,
        /// путешествия, профиль.
        case grouped
        /// Столько знаков, сколько число заслуживает: десятые ниже
        /// `unit.tenthsBelow`, дальше целое с разрядами — и НИКОГДА хвост
        /// «.0». Отметки, строка поездки в гараже, плитка чужого профиля,
        /// виджет, «дороги края».
        ///
        /// Хвост здесь роняется по той же причине, по которой выше порога
        /// роняются все десятые: у числа, которое уже сложилось и больше не
        /// изменится, «142.0» — не десятая, а след форматтера. До 0.6.7 это
        /// делал `GarageFormat.fuel`, и вместе с ним правило потерялось в двух
        /// местах сразу.
        case adaptive
    }
    ...
    private static func number(
        ...
        case .adaptive:
            guard unit.showsTenths(value) else { return grouped(value, lang) }
            let shown = tenths(value, lang)
            // Единственное место, где живёт правило «целое — без хвоста».
            // `grouped`, а не обрезание строки: вместе с хвостом обязано уйти и
            // `fractionDigits`, иначе подпись останется дробной формой — «5,0
            // мили» превратится в «5 мили» вместо «5 миль».
            return shown.value == shown.value.rounded() ? grouped(value, lang) : shown
        }
    }
