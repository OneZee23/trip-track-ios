...
///
/// **`placeId` — та самая точка, ставшая МЕСТОМ (0.6.8).** Отметка принадлежит
/// одной поездке, место — узнаётся на каждой поездке мимо, и «сколько до моря»
/// из одного числа превращается в историю: 2:14, 2:31, 2:08. Заполняет поле
/// `PlaceManager` ЛОКАЛЬНО — детерминированным id ячейки отметки
/// (`Place.id(forCell:)`), поэтому два телефона сходятся без синка, а провод и
/// миграция отметок не понадобились. `nil` — у отметки, которую ещё не
/// сверяли (и у надгробия наоборот: удалённое место остаётся здесь записанным,
/// чтобы сверка его не воскресила).
struct TripCheckpoint: Identifiable, Codable, Equatable {
    ...
    var photoIds: [UUID] = []
    /// Место, узнаваемое на каждом проезде (см. доккомент типа). С 0.6.8
    /// заполняется локально `PlaceManager` — id ячейки, `Place.id(forCell:)`;
    /// `nil` — отметка, которую ещё не сверяли.
    var placeId: UUID?
