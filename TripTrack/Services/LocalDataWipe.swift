...
enum LocalDataWipe {
    ...
    static func run() {
        ...
        // Строки `JourneyEntity` стёрты пакетным удалением, но список в памяти
        // о нём не знает: пакет идёт мимо контекста, а `@Published journeys`
        // читается только через `reload()`. Без этой строки «Мои» после
        // стирания рисуют карточки путешествий, у которых внутри уже ничего
        // нет, — и открываются они на пустой экран.
        JourneyManager.shared.reload()
        // Места и проезды — по той же причине: строки стёрты тем же пакетным
        // удалением мимо контекста, а `@Published places` читается только
