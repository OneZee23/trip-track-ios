...
enum VehiclePhotoStore {
    ...
    static func invalidateMainCache() {
        ...
        // Кэш КАРТИНОК здесь НЕ сбрасывается. Он ключуется по имени файла, а
        // имена — UUID и не переиспользуются: удаление одного снимка не делает
        // недействительными копии остальных. Сброс отсюда стоил дорого — он
        // случался в том числе при закрытии просмотрщика и заставлял экран
        // под ним разбирать все свои картинки заново.
    }
